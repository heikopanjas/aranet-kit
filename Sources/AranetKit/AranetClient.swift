...
public class AranetClient: NSObject, @unchecked Sendable {
    ...
    public func scan(timeout: TimeInterval = 10.0) async throws -> [AranetDevice] {
        ...
        let peripherals: [CBPeripheral] = try await withCheckedThrowingContinuation { [weak self] continuation in
            guard let self = self else { return }
...
extension AranetClient: CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        if verbose == true {
            ...
            print("[DEBUG] Services discovered: \(operation.servicesDiscovered)/\(operation.expectedServices)")
        }
